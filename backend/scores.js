...
// Map football-data.org's squad position label to our GK/DF/MF/FW buckets.
// Order matters: "Attacking Midfield" must resolve to MF, not FW.
function mapSquadPosition(label) {
  ...
  if (s.includes("forward") || s.includes("wing") || s.includes("striker") || s.includes("offen") || s.includes("attack")) return "FW";
  return null;
}

// Pull PL squads (players) + managers (coach) from football-data.org and merge into
// pl2627_players / pl2627_teams. Non-destructive: players are added with INSERT OR IGNORE
// (no deletes, so it never wipes users' award picks), and managers are refreshed from the
// API's coach field. Returns a diagnostic so the admin button can report what the API served
// — notably whether this API-key tier includes squad data at all.
async function syncPLSquads() {
  const apiKey = process.env.FOOTBALL_API_KEY;
  if (!apiKey) return { ok: false, reason: "no_api_key" };

  try {
    const res = await fetch(`${API_BASE}/competitions/${PL_COMPETITION}/teams?season=2026`, {
      headers: { "X-Auth-Token": apiKey },
    });
    ...
    const teamByCode = {};
    for (const t of db.prepare("SELECT id, code FROM pl2627_teams").all()) teamByCode[t.code] = t.id;

    const insertPlayer = db.prepare("INSERT OR IGNORE INTO pl2627_players (name, team_id, position) VALUES (?, ?, ?)");
    const updateManager = db.prepare("UPDATE pl2627_teams SET manager = ? WHERE id = ?");

    let squadTeams = 0, playersAdded = 0, apiSquadPlayers = 0, managersUpdated = 0, skippedPlayers = 0;
    const unknownTeams = new Set();

    const sync = db.transaction(() => {
      for (const t of teams) {
        const teamId = teamByCode[plCode(t.tla)];
        if (!teamId) { if (t.tla) unknownTeams.add(t.tla); continue; }

        if (t.coach?.name) { updateManager.run(t.coach.name, teamId); managersUpdated++; }

        const squad = Array.isArray(t.squad) ? t.squad : [];
        if (squad.length > 0) squadTeams++;
        for (const pl of squad) {
          apiSquadPlayers++;
          const pos = mapSquadPosition(pl.position);
          if (!pos || !pl.name) { skippedPlayers++; continue; }
          if (insertPlayer.run(pl.name, teamId, pos).changes) playersAdded++;
        }
      }
    });
    sync();
