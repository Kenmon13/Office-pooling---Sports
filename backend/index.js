...
// Fix: align an inconsistent KO pick's winner to its score by flipping the winner to the
// higher-scoring side (draws are never inconsistent, so exactly one side is higher). Same
// scope as the audit (all R32 + R16 Canada vs Morocco). Dry-run by default — pass {apply:true}
// to write. Each UPDATE re-checks the inconsistency in its WHERE clause, so it's idempotent
// (a rerun changes 0 rows) and only ever touches rows that still contradict their score.
app.post("/api/admin/ko-consistency-fix", requireAdminToken, (req, res) => {
  const apply = req.body && req.body.apply === true;
  const rows = db.prepare(`
    SELECT kp.participant_id, kp.match_id, m.round, th.name AS home, ta.name AS away,
           p.name AS who, kp.predicted_winner AS old_pick,
           kp.predicted_home_score AS ph, kp.predicted_away_score AS pa,
           CASE WHEN kp.predicted_home_score > kp.predicted_away_score THEN 'home' ELSE 'away' END AS new_pick
    FROM knockout_predictions kp
    JOIN knockout_matches m ON kp.match_id = m.id
    JOIN participants p     ON kp.participant_id = p.id
    LEFT JOIN teams th ON m.home_team_id = th.id
  ...
  if (apply && rows.length > 0) {
    const upd = db.prepare(`
      UPDATE knockout_predictions SET predicted_winner = ?
      WHERE participant_id = ? AND match_id = ?
        AND ((predicted_winner = 'home' AND predicted_home_score < predicted_away_score)
          OR (predicted_winner = 'away' AND predicted_away_score < predicted_home_score)
          OR (predicted_home_score > predicted_away_score AND predicted_winner <> 'home' AND predicted_winner <> 'away')
          OR (predicted_away_score > predicted_home_score AND predicted_winner <> 'home' AND predicted_winner <> 'away'))
    `);
    const tx = db.transaction((list) => {
      for (const r of list) changed += upd.run(r.new_pick, r.participant_id, r.match_id).changes;
    });
    tx(rows);
  }
